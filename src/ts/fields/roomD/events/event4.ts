...
export default Field.EventEntryFactory.create(4, [
  ...
  op
    .if(() => GameGlobal.ownItems.has(GameItemIds.FlashLight))(
    ...
    .elseIf(() => GameGlobal.ownItems.has(GameItemIds.Lighter) && GameGlobal.ownItems.has(GameItemIds.LighterOil))(
      cmd.message(texts.get(1)),
      cmd.choices(texts.get(2), [texts.get(3), texts.get(4)], GameVariableKeys.Choices, 1),

      op
        .if(() => GameGlobal.variables.get(GameVariableKeys.Choices) === 0)(
          cmd.message(texts.get(5)),
          cmd.cameraFadeOutAll(),
          cmd.flag(GameFlagKeys.EndingA, true),
          cmd.moveField(FieldIds.UndergroundPathway, 64, 352, Model.Direction.Down),
        )
        .else(cmd.message(texts.get(6))),
    )
    .elseIf(() => GameGlobal.ownItems.has(GameItemIds.Lighter))(cmd.message(texts.get(7)), cmd.message(texts.get(8)))
    .else(cmd.message(texts.get(9))),
]);
