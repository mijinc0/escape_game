...
export interface IActorSpriteFactory {
  createOneWayAnimActorSprite(x: number, y: number, spriteKey: string, initFrame: number, bodyConfig?: IBodyConfig): IActorSprite;
  
  createFourWayAnimsActorSprite(x: number, y: number, spriteKey: string, initFrame: number, bodyConfig?: IBodyConfig): IActorSprite;
